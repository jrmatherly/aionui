...
} from "./cli/useReactToolScheduler";
import {
	getPromptCount,
...
export class GeminiAgent {
	...
	/** Builtin skills directory path / Builtin skills directory path */
	...
	private initClientEnv() {
		...
		const getCurrentApiKey = () => {
			if (this.apiKeyManager?.hasMultipleKeys()) {
				return (
					process.env[this.apiKeyManager.getStatus().envKey] ||
					this.model.apiKey
				);
			}
		...
		// Clear all auth-related environment variables to avoid interference between different auth types
		...
		if (this.authType === AuthType.LOGIN_WITH_GOOGLE) {
			// For personal OAuth authentication, GOOGLE_CLOUD_PROJECT is not needed
			// If user configures an invalid project ID, it will cause a 403 permission error
			// Only set if user explicitly configured a valid project ID
			if (this.googleCloudProject?.trim()) {
				process.env.GOOGLE_CLOUD_PROJECT = this.googleCloudProject.trim();
			}
			// Note: LOGIN_WITH_GOOGLE uses OAuth, no need to set any API Key
			return;
		}
	...
	private enrichErrorMessage(errorMessage: string): string {
		const reportMatch = errorMessage.match(
			/Full report available at:\s*(.+?\.json)/i,
		);
	...
	private async initialize(): Promise<void> {
		...
		// Use the provided YOLO setting
		const yoloMode = this.yoloMode;

		// Initialize conversation-level tool configuration
		...
		console.log(
			`[GeminiAgent] presetRules length: ${this.presetRules?.length || 0}`,
		);
		if (this.presetRules) {
		...
		this.scheduler = new CoreToolScheduler({
			onAllToolCallsComplete: async (
				...
				try {
						...
						if (response.length > 0) {
							const geminiTools = completedToolCalls.filter((tc) => {
								...
								if (isTerminalState) {
									...
									return (
										completedOrCancelledCall.response?.responseParts !==
											undefined && !tc.request.isClientInitiated
									);
								}
								return false;
							});
					...
					this.onStreamEvent({
						type: "error",
						data:
							"handleCompletedTools error: " + (e.message || JSON.stringify(e)),
			...
			onToolCallsUpdate: (updatedCoreToolCalls: ToolCall[]) => {
				try {
					...
					const toolCalls: TrackedToolCall[] = updatedCoreToolCalls.map(
						(coreTc) => {
					...
					this.onStreamEvent({
						type: "error",
						data:
							"tool_calls_update error: " + (e.message || JSON.stringify(e)),
						msg_id: this.activeMsgId ?? uuid(),
					});
				}
			},
	...
	private handleMessage(
		...
		const onConnectionEvent = (event: StreamConnectionEvent) => {
			if (event.type === "heartbeat_timeout") {
				...
				console.error(
					`[GeminiAgent] Stream connection failed: ${event.reason}`,
				);
				this.onStreamEvent({
					type: "error",
		...
		return processGeminiStreamEvents(
			...
			.then(async () => {
				...
				if (
					...
					console.log(
						`[GeminiAgent] Retrying after invalid stream (attempt ${retryCount + 1})`,
					);
					...
					// Re-send message
					const prompt_id =
						this.config.getSessionId() + "########" + getPromptCount();
				...
				// If invalid_stream detected but cannot retry, show final error
	...
	submitQuery(
		...
		try {
			...
			// Emit start event immediately when stream is created to ensure UI shows stop button
	...
	async send(
		...
		// OAuth Token pre-check (only effective for OAuth mode)
		if (this.authType === AuthType.LOGIN_WITH_GOOGLE) {
			try {
				...
				if (!isTokenValid) {
					console.warn(
						"[GeminiAgent] OAuth token validation failed, proceeding anyway",
					);
				}
			} catch (tokenError) {
		...
		if (!this.skillsIndexPrependedOnce) {
			// Prefer presetRules, otherwise fallback to contextContent
		...
		// files parameter is only used for copying to workspace, not passing path hints to the model
		...
		const { processedQuery, shouldProceed } = await handleAtCommand({
			...
			// Enable lazy loading when files are provided: do not read file content immediately
		...
		if (
			...
			// If @ command processing failed, send error message to user
			if (atCommandError) {
				this.onStreamEvent({
					type: "error",
					data: atCommandError,
	...
	async injectConversationHistory(text: string): Promise<void> {
		try {
			...
			const { memoryContent } = await refreshServerHierarchicalMemory(
				this.config,
			);
