...
const CodexSendBox: React.FC<{ conversation_id: string }> = ({ conversation_id }) => {
  ...
  // Clean up thought throttle timer
  useEffect(() => {
    return () => {
  ...
  // Clean up all states when conversation ID changes to avoid pollution
  useEffect(() => {
  ...
  const onSendHandler = async (message: string) => {
    const msg_id = uuid();
    // Immediately clear input and selected files to improve UX
    setContent('');
    ...
    // Do not auto-add @ prefix to avoid line breaks and ambiguity
    ...
    // Frontend writes user message first to avoid navigation/event races causing invisible messages
    ...
    addOrUpdateMessage(userMessage, true); // Save immediately to storage to prevent loss on refresh
  ...
  // Handle initial message from guide page, wait for connection established before sending
  useEffect(() => {
    ...
    const processInitialMessage = async () => {
      ...
      // Double check lock mode to prevent race conditions
      ...
      try {
        ...
        // Remove initial message from storage on success
