      ...
      <div className='flex items-center justify-between gap-12px w-full' style={{ minWidth: 'max-content' }}>
        ...
        <div className='flex items-center h-full gap-12px'>
          ...
          {(isMarkdown || isHTML) && (
            <>
              ...
              <div
                ...
                title={isSplitScreenEnabled ? 'Close split screen' : 'Open split screen'}
              >
          ...
          {contentType === 'code' && isEditable && (
            <div className={`flex items-center gap-4px px-8px py-4px rd-4px cursor-pointer hover:bg-bg-3 transition-colors ${isEditMode ? 'bg-primary text-white' : ''}`} onClick={() => (isEditMode ? onExitEdit() : onEditClick())} title={isEditMode ? 'Exit Edit' : 'Edit'}>
              ...
              <span className='text-12px'>{isEditMode ? 'Exit Edit' : 'Edit'}</span>
            </div>
          )}
        ...
        <div className='flex items-center gap-8px flex-shrink-0'>
          ...
          {((contentType === 'markdown' && (viewMode === 'source' || isSplitScreenEnabled)) || (contentType === 'html' && (viewMode === 'source' || isSplitScreenEnabled)) || (contentType === 'code' && isEditable && isEditMode)) && (
            <>
              ...
              <div className={`flex items-center gap-4px px-8px py-4px rd-4px transition-colors ${historyTarget ? 'cursor-pointer hover:bg-bg-3' : 'cursor-not-allowed opacity-50'} ${snapshotSaving ? 'opacity-60' : ''}`} onClick={historyTarget && !snapshotSaving ? onSaveSnapshot : undefined} title={historyTarget ? 'Save current snapshot' : 'Current content does not support snapshot'}>
                ...
                <span className='text-12px text-t-secondary'>{'Snapshot'}</span>
              </div>

              {/* History button */}
              {historyTarget ? (
                <Dropdown droplist={renderHistoryDropdown()} trigger={['hover']} position='br' onVisibleChange={(visible) => visible && onRefreshHistory()}>
                  <div className='flex items-center gap-4px px-8px py-4px rd-4px cursor-pointer hover:bg-bg-3 transition-colors' title={'History Versions'}>
                    <svg width='14' height='14' viewBox='0 0 24 24' fill='none' stroke='currentColor' strokeWidth='1.8' className='text-t-secondary'>
                      <path d='M12 8v5l3 2' />
                      <path d='M12 3a9 9 0 1 0 9 9' />
                      <polyline points='21 3 21 9 15 9' />
                    </svg>
                    <span className='text-12px text-t-secondary'>{'History'}</span>
                  </div>
                </Dropdown>
              ) : (
                <div className='flex items-center gap-4px px-8px py-4px rd-4px cursor-not-allowed opacity-50 transition-colors' title={'Current content does not support history'}>
                  <svg width='14' height='14' viewBox='0 0 24 24' fill='none' stroke='currentColor' strokeWidth='1.8' className='text-t-secondary'>
                    <path d='M12 8v5l3 2' />
                    <path d='M12 3a9 9 0 1 0 9 9' />
                    <polyline points='21 3 21 9 15 9' />
          ...
          {showOpenInSystemButton && (
            <div className='flex items-center gap-4px px-8px py-4px rd-4px cursor-pointer hover:bg-bg-3 transition-colors' onClick={onOpenInSystem} title={'Open in system app'}>
              ...
              <span className='text-12px text-t-secondary'>{'Open in system app'}</span>
            </div>
          )}

          {/* Download button */}
          <div className='flex items-center gap-4px px-8px py-4px rd-4px cursor-pointer hover:bg-bg-3 transition-colors' onClick={() => void onDownload()} title={'Download file'}>
            <svg width='14' height='14' viewBox='0 0 24 24' fill='none' stroke='currentColor' strokeWidth='2' className='text-t-secondary'>
          ...
          {isHTML && onInspectModeToggle && (
            <div className={`flex items-center gap-4px px-8px py-4px rd-4px cursor-pointer transition-colors ${inspectMode ? 'bg-primary text-white' : 'hover:bg-bg-3'}`} onClick={onInspectModeToggle} title={inspectMode ? 'Disable element inspector' : 'Enable element inspector'}>
              ...
              <span className={`text-12px ${inspectMode ? 'text-white' : 'text-t-secondary'}`}>{inspectMode ? 'Inspecting...' : 'Inspect element'}</span>
            </div>
          )}
        </div>
      </div>
