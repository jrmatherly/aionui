...
/**
 * List of models explicitly not supporting certain capabilities - Blacklist
 */
...
/**
 * Check if the user has manually configured a capability type
 * @param model - Model object
 * @param type - Capability type
 * @returns true/false if explicitly configured by user, undefined if not configured
 */
...
/**
 * Get specific capability rules based on provider
...
/**
 * Determine if a model possesses a certain capability - Reference: Cherry Studio's 3-layer logic
 * @param model - Model object
 * @param type - Capability type
...
export const hasModelCapability = (model: IProvider, type: ModelType): boolean | undefined => {
  // Generate cache key (include capabilities version to avoid stale cache)
...
/**
 * Clear capability judgment cache
 */
