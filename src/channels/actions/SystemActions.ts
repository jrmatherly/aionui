...
export const handleSessionNew: ActionHandler = async (context) => {
  ...
  if (existingSession) {
    ...
    // Use session.conversationId directly to clean up the agent in WorkerManage
    // Ensure proper cleanup even if sessionConversationMap is empty
...
export const handleHelpShow: ActionHandler = async (context) => {
  ...
  return createSuccessResponse({
    type: 'text',
    text: [`❓ <b>${getBrandName()} Assistant</b>`, '', `A remote assistant to interact with ${getBrandName()} via Telegram.`, '', '<b>Common Actions:</b>', '• 🆕 New Chat - Start a new session', '• 📊 Status - View current session status', '• ❓ Help - Show this help message', '', 'Send a message to chat with the AI assistant.'].join('\n'),
    parseMode: 'HTML',
...
export const handleHelpPairing: ActionHandler = async (context) => {
  ...
  return createSuccessResponse({
    type: 'text',
    text: ['🔗 <b>Pairing Guide</b>', '', '<b>First-time Setup:</b>', '1. Send any message to the bot', '2. Bot displays pairing code', `3. Approve pairing in ${getBrandName()} settings`, '4. Ready to use after pairing', '', '<b>Notes:</b>', '• Pairing code valid for 10 minutes', `• ${getBrandName()} app must be running`, '• One Telegram account can only pair once'].join('\n'),
    parseMode: 'HTML',
    replyMarkup: createHelpKeyboard(),
  });
};
...
export const handleSettingsShow: ActionHandler = async (context) => {
  ...
  return createSuccessResponse({
    type: 'text',
    text: ['⚙️ <b>Settings</b>', '', `Channel settings need to be configured in the ${getBrandName()} app.`, '', `Open ${getBrandName()} → WebUI → Channels`].join('\n'),
    parseMode: 'HTML',
