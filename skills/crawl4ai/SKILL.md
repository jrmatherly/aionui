...
When running in Docker or headless environments (like AionUI web deployment):
...
```bash
# Install Playwright browsers (one-time, ~350MB)
...
**Note:** AionUI Docker images include Playwright system dependencies. Only the Chromium browser binary needs to be installed.
